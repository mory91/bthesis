...
lines_with_delimiter = [x for x in map(lambda x: x+'!',lines)] #put delimiter symbol
maxlen = max(map(lambda x: len(x),lines_with_delimiter)) #find maximum line size
# next lines here to get all possible characters for events and annotate them with numbers
chars = map(lambda x: set(x), lines_with_delimiter)
...
with open('output_files/folds/fold1.csv', 'w') as csvfile:
    ...
    for row, timeseq in zip(fold1, fold1_t):
        spamwriter.writerow([str(str(s).encode("utf-8")) +'#{}'.format(t) for s, t in zip(row, timeseq)])
